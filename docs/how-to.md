...
### Install shell completion scripts generated by `container`.

### Overview

The `container --generate-completion-script [zsh|bash|fish]` command generates completion scripts for the provided shell. 
Below is a detailed guide on how to install the completion scripts.

> [!NOTE]
> Reference was taken from the Argument Parser Documentation located [here](https://apple.github.io/swift-argument-parser/documentation/argumentparser/installingcompletionscripts/#Installing-Zsh-Completions).

### Installing Zsh Completions

If you have [oh-my-zsh](https://ohmyz.sh/) installed, you already have a directory of automatically loading completion scripts — `.oh-my-zsh/completions`. Copy your new completion script to that directory. If the `completions` directory does not exist, simply make it.
...
> [!NOTE]
> Your completion script must have the following filename format `_container`.
...
> [!NOTE]
> The path to the directory is dependent on how bash-completion was installed.
> You would need to find the correct path and then copy the completion script there.
> Below is an example path for if it was installed via homebrew.
