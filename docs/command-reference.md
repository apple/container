...
* `--scheme <scheme>`: Scheme to use when connecting to the container registry. One of (http, https, auto) (default: auto)
  ...
  * **Behavior of `auto`**
    ...
    * The host ends with the machine's default container DNS domain (as defined in `DNSConfig.defaultDomain`, located [in the `ContainerSystemConfig.swift` source](../Sources/ContainerPersistence/ContainerSystemConfig.swift))
