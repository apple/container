...
public final class TarArchiveDiffer: Differ {
    ...
    private let inspectorOptions: InspectorOptions
    private let directoryDiffer: DirectoryDiffer
    ...
    public init(contentStore: any ContentStore) {
        ...
        self.inspector = DefaultFileAttributeInspector()
        self.hasher = SHA256ContentHasher()
        self.inspectorOptions = InspectorOptions()
        self.directoryDiffer = DirectoryDiffer(
            inspector: self.inspector,
            fileDiffer: FileDiffer(
                contentDiffer: FileContentDiffer(hasher: self.hasher),
    ...
    func stageTarInputs(
        ...
        for change in changes {
                ...
                if FileManager.default.fileExists(atPath: sourceFile.path) {
                    ...
                    entriesToArchive.append(
                        Archiver.ArchiveEntryInfo(
                            ...
                            pathInArchive: URL(fileURLWithPath: path),
                            owner: a.uid,
                            group: a.gid,
                    ...
                    if inspectorOptions.enableXAttrsCapture {
                        ...
                        if !entries.isEmpty {
                            ...
                            let sidecarRel = ".container/xattrs/\(path).bin"
            ...
            case .deleted(let path):
                let whiteoutName = ".wh." + (path as NSString).lastPathComponent
                let whiteoutDir = (path as NSString).deletingLastPathComponent
                let whiteoutPath = whiteoutDir.isEmpty ? whiteoutName : "\(whiteoutDir)/\(whiteoutName)"

                let whiteoutFile = tempDir.appendingPathComponent(whiteoutPath)
                try FileManager.default.createDirectory(
                ...
                entriesToArchive.append(
                    Archiver.ArchiveEntryInfo(
                        ...
                        pathInArchive: URL(fileURLWithPath: whiteoutPath)
                    )
                )
            }
        }
    ...
    private func applyTarLayer(file: URL, mediaType: String?, to root: URL) throws {
        ...
        for (entry, data) in reader {
            ...
            if lastComponent.hasPrefix(".wh.") {
                ...
                let dirRel = (pathInArchive as NSString).deletingLastPathComponent
                let targetRel = dirRel.isEmpty ? removedName : "\(dirRel)/\(removedName)"
