...
public struct DirectoryDiffer: Sendable {
    ...
    // In some cases, a single file may be reachable through multiple
    // directory entries. Common causes include:
    // - Hard links: multiple names pointing at the same inode
    // - Bind mounts: the same subtree made visible at multiple paths
    // - Symlinks: links within the tree
    //
    // When traversing, especially with concurrent enumeration, these can yield multiple
    // diffs that would encode to the same logical DiffKey record. The order in which
    // such duplicates appear is non-deterministic across traversals and hosts.
    //
    // Therefore, we dedup prior to key materialization so downstream consumers
    // can reliably operate on a stable set.
