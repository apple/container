...
public struct FileMetadataDiffer: Sendable {
    ...
    public func diff(
        ...
        // For non-regular files, a size difference is treated as metadata-only
        let nonRegularSizeChanged = (!sizeChanged && old.type != .regular && new.type != .regular && (old.size ?? -1) != (new.size ?? -1))

        // Aggregate metadata-only fields
        let anyMetadataChange =
            modeChanged || uidChanged || gidChanged || mtimeChanged || ctimeChanged || xattrsDigestChanged || nonRegularSizeChanged
