...
public actor SandboxService {
    ...
    public func bootstrap(_ message: XPCMessage) async throws -> XPCMessage {
        ...
        return try await self.lock.withLock { _ in
            ...
            let stdout = MultiWriter(handles: [containerLog])
            ...
            let stdin = config.initProcess.openStdin ? DelayedReader() : nil
    ...
    private func startInitProcess(stdio: [FileHandle?], lock: AsyncLock.Context) async throws {
        ...
        do {
            ...
            if let stdin = stdio[0] {
                try io.in?.addAndStartReader(fh: stdin)
            }

            if let stdout = stdio[1] {
                io.out?.addHandle(handle: stdout)
            }

            if let stderr = stdio[2] {
                io.err?.addHandle(handle: stderr)
            }
    ...
    private func startExecProcess(processId id: String, stdio: [FileHandle?], lock: AsyncLock.Context) async throws {
        ...
        let czConfig = self.configureProcessConfig(config: processInfo.config, stdio: stdio)
...
final class MultiWriter: Writer {
    let handles: Mutex<[FileHandle]>

    init(handles: [FileHandle]) {
        self.handles = Mutex(handles)
    }

    func addHandle(handle: FileHandle) {
        self.handles.withLock {
            $0.append(handle)
        }
    }
...
final class DelayedReader: ReaderStream {
    private let state: Mutex<State>
    let cont: AsyncStream<Data>.Continuation
    let aStream: AsyncStream<Data>
    ...
    func addAndStartReader(fh: FileHandle) throws {
        self.state.withLock {
            $0.handle = fh
...
extension SandboxService {
    ...
    private struct ContainerInfo {
        ...
        let io: (in: DelayedReader?, out: MultiWriter?, err: MultiWriter?)
    }
