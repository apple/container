    ...
    public struct ContainerCopy: AsyncLoggableCommand {
        ...
        public func run() async throws {
            ...
            case (.container(let id, let path), .local(let localPath)):
                ...
                let destPath = FilePath(URL(fileURLWithPath: localPath).standardized.path)
            ...
            case (.local(let localPath), .container(let id, let path)):
                let srcPath = FilePath(URL(fileURLWithPath: localPath).standardized.path)
                var isDirectory: ObjCBool = false

                guard let srcComponent = srcPath.lastComponent else {
                ...
                let destPath = path.hasSuffix("/") ? path.appending(srcComponent.string) : path
