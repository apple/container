...
actor VolumesService {
    ...
    public func setContainersService(_ containersService: ContainersService) {
    ...
    private func parseSize(_ sizeString: String) throws -> UInt64 {
        ...
        let maxSize: UInt64 = 1.tib()  // 1tib maximum
        ...
        guard sizeInBytes <= maxSize else {
            throw VolumeError.storageError("Volume size too large: maximum 1TiB")
        }
    ...
    private func _delete(name: String) async throws {
        ...
        if let containersService = self.containersService, await containersService.isVolumeInUse(name) {
