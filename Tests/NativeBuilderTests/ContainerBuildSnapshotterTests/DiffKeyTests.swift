...
@Suite struct DiffKeyIntegrationTests {
    ...
    @Test func deterministicForSameTree() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let k1 = try await TestUtils.computeDiffKey(targetMount: t1)
            let k2 = try await TestUtils.computeDiffKey(targetMount: t2)
    ...
    @Test func contentChangeAltersKey() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let k1 = try await TestUtils.computeDiffKey(targetMount: t)

            // Change file bytes
            try TestUtils.writeString(f, "HELLO")
            let k2 = try await TestUtils.computeDiffKey(targetMount: t)
    ...
    @Test func baseDigestAffectsKey() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let kScratch = try await TestUtils.computeDiffKey(targetMount: t)
            let kB1 = try await TestUtils.computeDiffKey(baseDigest: base1, targetMount: t)
            let kB2 = try await TestUtils.computeDiffKey(baseDigest: base2, targetMount: t)
    ...
    @Test func additionDeletionSymlinkTargetImpact() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let kAdd = try await TestUtils.computeDiffKey(targetMount: t)
            #expect(k1 != kAdd)

            // Remove it again
            try FileManager.default.removeItem(at: t.appendingPathComponent("b.txt"))
            let kDel = try await TestUtils.computeDiffKey(targetMount: t)
            #expect(kAdd != kDel)
            ...
            let kL1 = try await TestUtils.computeDiffKey(targetMount: t)
            ...
            let kL2 = try await TestUtils.computeDiffKey(targetMount: t)
            #expect(kL1 != kL2)
        }
    }
}
