...
@Suite struct DiffKeyTests {
    ...
    @Test func deterministicForSameTree() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let k1 = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t1)
            let k2 = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t2)
    ...
    @Test func contentChangeAltersKey() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let k1 = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t)

            // Change file bytes
            try TestUtils.writeString(f, "HELLO")
            let k2 = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t)
    ...
    @Test func baseDigestAffectsKey() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let kScratch = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t)
            let kB1 = try await DiffKey.compute(baseDigest: base1, baseMount: nil, targetMount: t)
            let kB2 = try await DiffKey.compute(baseDigest: base2, baseMount: nil, targetMount: t)
    ...
    @Test func additionDeletionSymlinkTargetImpact() async throws {
        try await TestUtils.withTempDirAsync { dir in
            ...
            let kAdd = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t)
            #expect(k1 != kAdd)

            // Remove it again
            try FileManager.default.removeItem(at: t.appendingPathComponent("b.txt"))
            let kDel = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t)
            #expect(kAdd != kDel)
            ...
            let kL1 = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t)
            ...
            let kL2 = try await DiffKey.compute(baseDigest: nil, baseMount: nil, targetMount: t)
            #expect(kL1 != kL2)
        }
    }
}
